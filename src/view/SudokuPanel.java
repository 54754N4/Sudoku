...
public class SudokuPanel extends JPanel implements MouseListener {
  ...
  private void setAutoBindings() {
    ...
    bind(BACKSPACE, KeyStroke.getKeyStroke(KeyEvent.VK_BACK_SPACE, 0), deleteSingle);
		bind(SHIFT.concat(DELETE), KeyStroke.getKeyStroke(KeyEvent.VK_DELETE, InputEvent.SHIFT_DOWN_MASK), deleteAll);
    ...
    bind(SHIFT.concat(LEFT), KeyStroke.getKeyStroke(KeyEvent.VK_LEFT, InputEvent.SHIFT_DOWN_MASK), left);
    bind(RIGHT, KeyStroke.getKeyStroke(KeyEvent.VK_RIGHT, 0), right);
    bind(SHIFT.concat(RIGHT), KeyStroke.getKeyStroke(KeyEvent.VK_RIGHT, InputEvent.SHIFT_DOWN_MASK), right);
    ...
    bind(SHIFT.concat(DOWN), KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, InputEvent.SHIFT_DOWN_MASK), down);
	}
    ...
    IntStream.range(0, NUMBERS.length)
        .forEach(i -> {
          ...
          bind(SHIFT.concat(NUMPAD).concat(name), KeyStroke.getKeyStroke("shift NUMPAD" + number), noteAction);
        });
	}
